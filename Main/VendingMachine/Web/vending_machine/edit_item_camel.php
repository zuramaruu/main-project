...
<div class="row" style="height:  100vh">
    ...
    <div class="col-8 p-5   ">
        ...
        if (isset($_GET["submit"]) and $_GET["submit"] == "submit") {
            ...
            $camel_insert = $_GET["camel_form"];
            $total_camel = 0;

            $total_camel = $camel_insert + getStock("camel", $IDBoard, "list_item", $koneksi);
            if ($total_camel > 4) {
                $valid = 2;
            } else {

                $sql = "UPDATE list_item  SET camel=? WHERE id_board = ?";
                if ($stmt = $koneksi->prepare($sql)) {

                    $stmt->bind_param("ii", $total_camel, $IDBoard);
                    if ($stmt->execute()) {
                    ...
                    <div class="card-body text-center">
                        ...
                        <form>
                            <div class="mb-3 form-group">
                                <input type="number" class="form-control rounded-left" id="camel_form" name="camel_form"
